...
const components = {};
function componentsPlugin_2fa1b366(nuxtApp) {
...
const vueuseHead_6918e46b = defineNuxtPlugin((nuxtApp) => {
...
const plugin_4342d25f = defineNuxtPlugin((nuxtApp) => {
...
const router_22adb880 = defineNuxtPlugin((nuxtApp) => {
  const routes = [];
  const hooks = {
    "navigate:before": [],
    "resolve:before": [],
...
class BasicCharacterController {
  ...
  _LoadModels() {
    ...
    loader.load("DungeonCrawler_Character.fbx", (fbx) => {
      ...
      fbx.traverse((c) => {
        ...
        if (c.geometry) {
          ...
          for (let i = 0; i < vertices.count; i += 3) {
            verts.push(new CANNON.Vec3(vertices.array[i], vertices.array[i + 1], vertices.array[i + 2]));
          }
      ...
      loader2.load("Standing_Idle.fbx", (a) => {
        _OnLoad("idle", a);
      });
    });
  }
...
const _sfc_main = {
  ...
  setup(__props) {
    class CharacterControllerDemo {
      ...
      _Initialize() {
        ...
        window.addEventListener("click", (event) => {
          ...
          const point = this._pickHelper.pick({
            x: (event.clientX - rect.left) / this._threejs.domElement.clientWidth * 2 - 1,
            y: (event.clientY - rect.top) / this._threejs.domElement.clientHeight * -2 + 1
        ...
        light = new THREE.AmbientLight(16777215, 0.25);
        this._scene.add(light);
        const controls = new OrbitControls(this._camera, this._threejs.domElement);
        controls.target.set(0, 10, 0);
        controls.update();
        const plane = new THREE.Mesh(new THREE.PlaneGeometry(100, 100, 10, 10), new THREE.MeshStandardMaterial({
          color: 8421504
        }));
      ...
      _Step(timeElapsed) {
        const timeElapsedS = timeElapsed * 1e-3;
        if (this._mixers) {
